...
STRONG void huart_wait_txdone(void)
{
...
STRONG void huart_set_baudrate(uint baud_rate)
{
...
STRONG uint huart_get_rxcnt(void)
{
...
///HUART 获取1Byte数据
AT(.com_text.huart.huart_getchar)
STRONG char huart_getchar(void)
{
    char data = 0;
...
///HUART 发送数据
AT(.com_text.huart.text)
STRONG void huart_tx(const void *buf, uint len)
{
    huart_wait_txdone();

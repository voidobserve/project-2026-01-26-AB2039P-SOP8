...
AT(.com_text.timer)
STRONG void usr_tmr1ms_isr_callback(void)
{
    uart_timeout_1ms_isr();
}
#endif
...
#if SYS_PWROFF_TIME
    sys_cb.pwroff_time = (u32)SYS_PWROFF_TIME * 10;    //100ms为单位
...
void power_on_check(void)
{
    ...
    u32 wakeup_reason = WK_LP_WK0;

#if LP_PWROFF_EN
    power_on_vbat_check();
#endif

    if (sys_cb.wakeup_reason != wakeup_reason || power_on_check_time == 0) {
        return;
    }

...
ALWAYS_INLINE void list_push_back_critical(list_t *list, list_hdr_t *list_hdr)
{
    GLOBAL_INT_DISABLE();
    list_push_back(list, list_hdr);
    GLOBAL_INT_RESTORE();
}

ALWAYS_INLINE list_hdr_t *list_pop_front_critical(list_t *list)
{
    GLOBAL_INT_DISABLE();
    list_hdr_t *hdr = list_pop_front(list);
    GLOBAL_INT_RESTORE();
    return hdr;
}

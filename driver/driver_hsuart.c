...
void hsuart_init(hsuart_init_typedef *hsuart_init_struct)
{
    ...
    } else if (hsuart_init_struct->clk_source == HSUT_UART0_PCLKPP_CLK) {
        clk_hsut0_clk_set(HSUT_UART0_PCLKPP_CLK, 0);
    }
    ...
    /* hsuart hardware flow control configuration */
    tmp_reg = HSUART->flow_ctrl;
    tmp_reg &= ~(HSUT0FCCON_RTS_MODE | HSUT0FCCON_URTS_EN | HSUT0FCCON_UCTS_EN);
    tmp_reg |= hsuart_init_struct->hardware_flow_ctrl_mode;
    HSUART->flow_ctrl = tmp_reg;
}
...
void hsuart_pic_config(isr_t isr, int pr, HSUART_IT_TYPEDEF interrupt_type, FUNCTIONAL_STATE state)
{
    ...
    uint32_t all_interrupt_type_mask = HSUART_IT_RX_DMA_HF | HSUART_IT_TX | HSUART_IT_RX | HSUART_IT_CTS;
    ...
    if (state != DISABLE) {
        ...
        HSUART->flow_ctrl |= (interrupt_type & HSUART_IT_CTS)? HSUT0FCCON_CTSIE: 0x00;
    } else {
        HSUART->con &= ~interrupt_pending_bit;
        HSUART->flow_ctrl &= ~ ((interrupt_type & HSUART_IT_CTS)? HSUT0FCCON_CTSIE: 0x00);
...
FLAG_STATE hsuart_get_flag(HSUART_IT_TYPEDEF interrupt_type)
{
    ...
    if ((HSUART->con & interrupt_pending_bit) != RESET) {
        return SET;
    } else if ((interrupt_type & HSUART_IT_CTS) && HSUART->flow_ctrl & HSUT0FCCON_CTSPND) {
        return SET;
    } else {
        return RESET;
    }
...
void hsuart_clear_flag(HSUART_IT_TYPEDEF interrupt_type)
{
    ...
    if (interrupt_type & HSUART_IT_CTS) {
        interrupt_pending_bit |= HSUT0CPND_CCTSPND;
    }
...
/**
  * @brief  hardware flow ctrl rts config.
  * @param  rts: SET or RESET.
  * @retval None
  */
AT(.com_periph.hsuart.hfc)
void hsuart_hardware_flow_ctrl_rts_config(FLAG_STATE rts)
{
    if (HSUART->flow_ctrl & (HSUT0FCCON_RTS_MODE | HSUT0FCCON_URTS_EN)) {
        if (rts != RESET) {
            HSUART->flow_ctrl |= HSUT0FCCON_RTS_SW;
        } else {
            HSUART->flow_ctrl &= ~HSUT0FCCON_RTS_SW;
        }
    }
...
/**
  * @brief  hardware flow ctrl rts status get.
  * @param  None
  * @retval FLAG_STATE: SET or RESET.
  */
AT(.com_periph.hsuart.hfc)
FLAG_STATE hsuart_hardware_flow_ctrl_rts_get(void)
{
    return (HSUART->flow_ctrl & HSUT0FCCON_RTSS)? SET: RESET;
}

/**
  * @brief  hardware flow ctrl cts status get.
  * @param  None
  * @retval FLAG_STATE: SET or RESET.
  */
AT(.com_periph.hsuart.hfc)
...
{
    return (HSUART->flow_ctrl & HSUT0FCCON_CTSS)? SET: RESET;
}
